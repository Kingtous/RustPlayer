...
impl Stream {
    ...
    fn get_codec(&self) -> Result<Codec, Error> {
        ...
        let codec: *const AVCodec = unsafe { avcodec_find_decoder(codec_id) };
        if codec.is_null() {
...
impl CodecContext {
    fn new(inner: *mut AVCodecContext, codec: *const AVCodec) -> CodecContext {
...
impl Codec {
    fn new(inner: *const AVCodec) -> Codec {
        Codec { inner }
    }
